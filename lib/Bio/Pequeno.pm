...
sub BUILD {
    ...
    my (
        $input_files,$kraken_db,      $help, $cpus,$minimum_gene_id_threshold,
    );
...
sub run {
	...
	for my $file (@{$self->input_files})
	{
		...
		my $unclassified_genes_obj = Bio::Pequeno::KrakenUnclassifiedRegions->new(fasta_file => $coding->fasta_file, 
		cpus => $self->cpus, 
